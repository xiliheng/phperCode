...
class StockInfo
{
	...
	public static function checkRunTime($basePath, $checkFlag = true)
	{
		###path
		$runtimePath = $basePath . "/data";
		$runDateFile = $runtimePath . "/runDate.txt";

		$todayDate = date("Y-m-d");
		$weekDate = date("w");
		if(($weekDate == 5) || ($weekDate == 6) || Tool::isFileHas($runDateFile,$todayDate))
		{
			Tool::log_print("Notice", __FILE__ . " 当天闭市，程序不运行");
			return false;
		}

		###优先判断当前时间在9:15到15:00之间
		if($checkFlag)
		{
			$nowMin = date("Hi");
			if ($nowMin < "0930" || $nowMin > "1505")
			{
				Tool::log_print("Notice", __FILE__ . " 不在开盘时间");
				return false;
			}
		}
